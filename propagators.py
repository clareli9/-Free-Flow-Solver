...
#the implementations are mainly coded according to the pseudocode given in lecture slides
def FCCheck(C,X):
    lists =[]
    value = X.cur_domain()
    #print(len(value))
    for d in value:
        ...
        for var in C.get_scope():
            if var == X:
               ...
               value_test.append(var.get_assigned_value())
        #print(value_test)
        if not C.check(value_test):
           #print("======================================")
           #print(value_test)
           lists.append((X,d))
           X.prune_value(d)
            ...
            for d in domain_var:
                if not c.has_support(var,d):
                   ...
                   if var.cur_domain_size() == 0:
                      ...
                      for c_prime in csp.get_all_cons():
                          if c_prime not in gac_queue and var in c_prime.get_scope():
